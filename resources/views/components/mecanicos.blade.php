<div id="modal-todos-mecanicos" class="hidden fixed inset-0 bg-black/20 flex items-center justify-center z-30 p-2 md:p-4">
    <div class="bg-white rounded-xl w-auto max-h-[90vh] shadow-2xl p-3 space-y-6">

        <!-- Encabezado -->
        <div class="flex justify-between items-center pb-2 border-b">
            <h2 class="text-xl font-bold text-gray-800">Lista completa de mecánicos</h2>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">RUC / CI</th>
                        <th scope="col" class="px-6 py-3">Teléfono</th>
                        <th scope="col" class="px-6 py-3">Vehículos Referidos</th>
                ...
                <tbody id="todos-mecanicos-table-body">
                    @foreach ($mecanicos ?? [] as $mecanico)
                        <tr data-id="{{ $mecanico->id }}" class="tr-mecanicos bg-white border-b border-gray-200">
                            ...
                            <td class="px-6 py-4">{{ $mecanico->ruc_ci ?? 'N/A' }}</td>
                            <td class="px-6 py-4">{{ $mecanico->telefono ?? 'N/A' }}</td>
                            <td class="px-6 py-4">
                                <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs font-medium">
                                    {{ $mecanico->vehiculos_referidos_count ?? 0 }}
                                </span>
                            </td>
                            <td class="px-6 py-4">{{ format_time($mecanico->created_at) }}</td>
                            <td class="px-6 py-4">
