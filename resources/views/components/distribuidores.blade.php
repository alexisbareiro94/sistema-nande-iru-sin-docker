<div id="modal-distribuidores" class="hidden flex inset-0 fixed z-50 justify-center items-center w-full h-full bg-black/40">
    <div class="relative p-4 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="relative bg-white rounded-lg shadow-sm">
            <!-- Modal header -->
            <div class=" items-center w-full justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                ...
                <div class=" flex">
                    ...
                    <button id="cerrar-q" class="hidden absolute z-50 right-10 top-1 mr-1 text-gray-400 px-1 py-1 transition-all duration-150 hover:bg-gray-300 hover:text-gray-700 rounded-md cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="size-6">
                        ...
                        <tbody id="body-tabla-distribuidores-dos">
                            @foreach ($distribuidores as $distribuidor)
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">{{ $distribuidor->ruc ?? 'No se registro RUC' }}</td>
                                    <td class="px-6 py-4">{{ $distribuidor->celular ?? 'No se registro Celular' }}</td>
                                    <td class="px-6 py-4">{{ $distribuidor->direccion ?? 'No se registro Direccion' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <button type="button"
