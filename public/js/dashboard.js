...
document.addEventListener('DOMContentLoaded', function () {
    // Inicializar gráfico de formas de pago
    initFormasPagoChart();
...
async function fetchDashboardData(periodo) {
    try {
        ...
        if (result.success) {
            ...
            updateFormasPago(result.datos.formas_pago);
...
function updateResumen(resumen) {
    document.getElementById('total-ingresos').textContent =
        `Gs. ${resumen.total_ingresos.toLocaleString('es-PY')}`;
    document.getElementById('total-egresos').textContent =
        `Gs. ${resumen.total_egresos.toLocaleString('es-PY')}`;
    document.getElementById('balance').textContent =
        `Gs. ${resumen.balance.toLocaleString('es-PY')}`;
    document.getElementById('cantidad-ventas').textContent =
        resumen.cantidad_ventas;
}

/**
 * Actualizar tabla de formas de pago
 */
function updateFormasPago(formasPago) {
...
function showLoading() {
    // Agregar opacity a las cards mientras carga
